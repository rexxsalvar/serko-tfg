    ...
    <div class="grid gap-4">
        @foreach ($events as $event)
            <article class="serko-card flex flex-col gap-4 px-6 py-5 md:flex-row md:items-center md:justify-between">
                <div>
                    <h2 class="text-xl font-bold">{{ $event->homeTeam->name }} vs {{ $event->awayTeam->name }}</h2>
                    <p class="text-sm text-slate-600">{{ $event->stadium->name }} - {{ $event->date->format('d/m/Y H:i') }}</p>
                </div>
